...
model = 'model'
output = 'output'
data_train_gt = 'Data_modified_rescaled/Data_gt/train_gt/'
data_train_im = 'Data_modified_rescaled/Data_im/train_im/'
data_train_index = 'Data_modified_rescaled/dir_name.txt'
...
def inference_bn(images):
    ...
    with tf.variable_scope('pool_msb_con4') as scope:
        pool_msb_con4 = tf.nn.max_pool(msb_con4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME',
                                       name=scope.name)
        kernel = _variable_with_weight_decay('weights', shape=[3, 3, 128, 128], stddev=0.01, wd=0.0005)
        con_p1 = tf.nn.conv2d(msb_con4, kernel, [1, 2, 2, 1], padding='SAME', name=scope.name)
        concat = tf.concat([pool_msb_con4, con_p1], 3, name=scope.name)

    # msb_con5
    with tf.variable_scope('msb_con5'):
        msb_con5 = multi_scale_block(concat, 256, 64, is_bn=True)
